...
void Mesh::addFace(Point3& v0, Point3& v1, Point3& v2)
{
    ...
    FPoint3 vector1 = vertices[face.vertex_index[1]].p - vertices[face.vertex_index[0]].p;
    FPoint3 vector2 = vertices[face.vertex_index[2]].p - vertices[face.vertex_index[0]].p;
    FPoint3 normal = vector1.cross(vector2);
...
//std::vector<int> Mesh::getFaceIdxWithPoints(int idx0, int idx1, int notFaceIdx, int notFaceVertexIdx) const
{
    ...
    if (candidateFaces.size() % 2 == 0)
    {
		...
		printf("Warning! Edge with uneven number of faces connecting it!(%lu)\n", candidateFaces.size() + 1);
        if (!has_disconnected_faces)
        {
            //cura::logWarning("Mesh has disconnected faces!\n");
			printf("Mesh has disconnected faces!\n");
