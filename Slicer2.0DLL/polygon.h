...
public:
	...
	const cura::Point& operator[] (unsigned int index) const
	{
	...
	{
		int64_t length = 0;
		cura::Point p0 = (*path)[path->size() - 1];
		for (unsigned int n = 0; n < path->size(); n++)
		{
			cura::Point p1 = (*path)[n];
			length += cura::vSize(p0 - p1);
			p0 = p1;
		}
		return length;
	}
...
class PolygonSingle : public PolygonRef
{
...
public:
	...
	PolygonSingle(PolygonRef& other)
    : PolygonRef(poly)
    {
        poly = *other.path;
    }

    std::vector<int> pointIdx_to_faceIdx;
};
...
public:
	ClipperLib::Paths paths;
	std::vector<bool> polyIsHole;
    std::vector<std::vector<int>> allFaceIdx;
    unsigned int size() const
    {
        return paths.size();
    }
    ...
    void add(PolygonSingle& poly)
    {
        paths.push_back(*poly.path);
        //allFaceIdx.push_back(poly.pointIdx_to_faceIdx);
    }
//#undef Polygon
    //void add(Polygon&& other_poly)
	void add(PolygonSingle&& other_poly)
    {
	...
	PolygonSingle convexHull() const;
	...
	*/
    ...
    //void simplify(int smallest_line_segment = MM2INT(10), int allowed_error_distance = MM2INT(5))
	{
...
private:
	...
	Point getCenterOfPolygon(ClipperLib::Path poly);
public:
    /*!
     * Split up the polygons into groups according to the even-odd rule.
...
public:
    ...
    {               
        ...
        for(unsigned int i=0; i<size(); i++)
        {
			...
			double area = INT2MM2(INT2MM2(fabs(thiss[i].area())));
	...
	Polygons remove(const Polygons& to_be_removed, int same_distance = 0) 
    {
        Polygons result;
