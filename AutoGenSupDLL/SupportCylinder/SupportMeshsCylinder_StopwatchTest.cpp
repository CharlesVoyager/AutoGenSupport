...
namespace XYZSupport
{
	...
	void SupportMeshsCylinder::Generate2(const vector<unique_ptr<SupportSymbol>> &symbols)
	{
		...
		for (int i = 0; i < vecSw.size(); i++)
		{
			DbgMsg(__FUNCTION__"===> vecSw[%d] time: %ld (ms)", i, vecSw[i].ElapsedMilliseconds());
		}
		DbgMsg(__FUNCTION__"===> g_sw1 time: %ld (ms)", g_sw1.ElapsedMilliseconds());
		DbgMsg(__FUNCTION__"===> g_sw2 time: %ld (ms)", g_sw2.ElapsedMilliseconds());
		DbgMsg(__FUNCTION__"===> g_sw3 time: %ld (ms)", g_sw3.ElapsedMilliseconds());
	...
	void SupportMeshsCylinder::genSupFromList(const vector<unique_ptr<SupportSymbol>> &ptsCones)
	{
		...
		///////////////////////////////////////////////////////////////////////
		for (int i = (int)SupSubBodyStartEndList.size() - 1; i >= 0; i--)				//******************************** 111111111111111111111111
		{
	...
	bool SupportMeshsCylinder::GetTouchPerpendicularRHVector3forCheckPoinDiatance(SupportPoint &point, const CubeMatrix &modelcube, /*out*/ bool &IsMiniSUp)
	{
		...
		///與物件距離太近長內部支撐
		if (RayCollisionDetect::getPickTouchNearest2(cubeSpaceforNewsup, Cubedata2, point.GetPosition(), point.GetDirection(), normThreshold, /*out*/ touchWor, /*out*/ touchNormal))	//*********
		{
