...
History:
1.7.0.14:
Referenced from Trunk - XYZware SVN: 13566

1.7.0.20:
Tim updated AddInternalAverage(), AddNoHoleInternalAverage(). 
XYZware Trunk SVN: 14302

1.7.0.22:
1. Fixed a bug causing model and support are separated
2. Change support density(low)
XYZware Trunk SVN: 14494
*/
#include "stdafx.h"
#include <map>
...
// libSlicer20.a
extern "C" int fnSlicer20DLL(const wchar_t *, double);

#endif

namespace XYZSupport
{
	vector<SupportPoint> SupportPointsList::GenerateAuto(	const wstring density,			// "LOW", "MED", "HIGH"
															double contactSize,				// the radius of tocuh point. Default is 0.4mm.
															const vector<int> &thresholds,
															const vector<unsigned char> &vecModelMeshWor,	// model's meshes in STL format
															...
															PERCENTAGE_CALLBACK callback)
	{
		...
		if (callback) progressSupportPointAuto = callback;

		vector<Triple<double, RHVector3, RHVector3>> candSupPosList;		// TouchRadius / Position / Direction
		vector<SupportPoint> vSupportPoints;									// C#: this.Clear();
...
#elif __APPLE__
		wstring pathForPictureStl = wTempFolder + wFilename + L".stl";
#endif
		wstring pathForXYZMeshDll = L"";
		PolygonPoint xyMinLaction;

		if (false == ReducedTriangles(&vecModelMeshWor[0], pathForXYZMeshDll, pathForPictureStl, xyMinLaction))
		{
			DbgMsg(__FUNCTION__"[ERROR] Reduced Facets failure..");
			return vSupportPoints;
		}
		...
		GetModuleFileName(GetModuleHandle(L"XYZSupport.dll"), wszCurDir, MAX_PATH);//get current path
		wchar_t *pwszSlash = wcsrchr(wszCurDir, '\\');
		if (pwszSlash)
			pwszSlash[0] = NULL;
		else
			wszCurDir[0] = NULL;

		wchar_t pathForSlicerDll[MAX_PATH] = L"";
		wsprintf(pathForSlicerDll, L"%s\\%s", wszCurDir, L"Slicer2.0DLL.dll");

		DbgMsgW(TEXT(__FUNCTION__) L"===> pathForSlicerDll: %s", pathForSlicerDll);
		...
		progressSupportPointAuto(50.0, isCancelled);				// slice finishes.
		OnGenerateMeshsProcessRateUpdate(20.0, isCancelled);		// 0 - 20: slice finishes. 20 ~ 50: SupportPointsList finishes.
		if (isCancelled == true) return vSupportPoints;
		...
		sw.Stop();
		DbgMsg(__FUNCTION__"===> FindNecessarySupportPointsNew time: %ld (ms)", sw.ElapsedMilliseconds());

		if (candSupPosList.size()==0)
		{
...
#if 1	// Delete all temporarily files
...
#endif
		// dump the result
		//SupportPointsListDbg::SupportPointsToFile(vSupportPoints, "SupportPoints.txt");

		DbgMsg(__FUNCTION__"===> Total Support Points: %d", vSupportPoints.size());
	...
	bool SupportPointsList::ReducedTriangles(const unsigned char* stlBufferWor, const wstring wstrPathForXYZMeshDll, const wstring pathForPictureStl, PolygonPoint& xyMinLaction)
	{
		...
		// To Do: Reduced triangles when the number of facet is large, for example, exceed 10000.
	...
	void SupportPointsList::FindNecessarySupportPointsNew(	double size, 
															/*out*/ vector<Triple<double, RHVector3, RHVector3>> &candSupPosList)
	{
		...
		{
			progressSupportPointAuto(50.0 + ((level+1) * 50.0 / layerModel.alllayer.size()), isCancelled);
			OnGenerateMeshsProcessRateUpdate(20.0 + ((level + 1) * 30.0 / layerModel.alllayer.size()), isCancelled);
			...
			pastLevelSupportPoints.clear();
			for(const auto &pt : nowLevelSupportPoint)	
			{
				pastLevelSupportPoints.push_back(pt);
			}
			pastLevelSupportPointsArea.clear();
			for (const auto &polygons : nowLevelSupportPointsArea)	
			{
				pastLevelSupportPointsArea.push_back(polygons);
			}
			downAreaThreshold.clear();
			for (const auto &key : upperAreaThreshold)	
			{
				downAreaThreshold.push_back(key);
			}
			lowerSolid.clear();
			for (const auto &one : upperSolid)	
			{
				lowerSolid.push_back(one);
			}
			modifiedLowerHole.clear();
			for (const auto &one : modifiedUpperHole)	
			{
				modifiedLowerHole.push_back(one);
			}
			supportPointToPolygonTable.clear();
			for (const auto &poly : nowSupportPointToPolygonTable)
			{
		...
		for(const auto &point: allGeneratedSupportPoint) //foreach(SupportPoint point in allGeneratedSupportPoint)
		{
			candSupPosList.push_back(Triple<double, RHVector3, RHVector3>(size, point.GetPosition(), point.GetDirection()));

			unique_ptr<SupportData> symbol(new SupportData());
			symbol->supType = (int)ModelType::MARK;
			symbol->radius1 = size;
			symbol->SetPosition(Coord3D((float)point.GetPosition().x, (float)point.GetPosition().y, (float)point.GetPosition().z));
			symbol->orientation = point.GetDirection();
			symbol->UpdateMatrix();

			_items2.push_back(move(symbol));
		}
	}
...
#elif __APPLE__	// On Mac OS, calling CleanPolygons() will cause crash. Don't know reason. Use the code below instead.
		output.resize(sol.size());
		for (int i = 0; i < sol.size(); ++i)
			CleanPolygon(sol[i], output[i], 1.415);
#endif
		return output;
	}
	//3
		...
		const vector<vector<IntPoint>> &clips, ClipType clipType, PolyFillType subtype, PolyFillType cliptype)
	{
		...
		PolyNode *polyNode = sol.GetFirst();
		if (polyNode != nullptr)
		{
			do 
			{
				bool test = polyNode->IsHole();
				if (test == true)
				{
					vector<IntPoint> poly;
					CleanPolygon(polyNode->Contour, poly, 1.415);
					if (poly.size() > 0)
							hole.push_back(poly);
					//hole.Add(sol.m_AllPolys[i].Contour);
				}
			...
			} while ((polyNode = polyNode->GetNext()));
		}
		output.push_back(solid);
		output.push_back(hole);
		return output;
	...
	{
		int meshid = 0;
		bool getMeshId = GetTriangleId(point, height, /*out*/ meshid);					
		RHVector3 newLocation(0, 0, 0);
		if (getMeshId == false)
		{
			meshid = FindMeshId(point, level);						
			newLocation = FindNewLocation(point, height, meshid, size);						
		}
		else
		{
			newLocation = FindNewLocation(point, meshModel.Meshs[meshid].Vector3ds[0].z, meshid, size);
		}
		RHVector3 normalVector = AddSupportPoint(newLocation, isDensityTooHigh, meshid, size, count);	
		return normalVector;
	}
		...
		if (h > 1.1)
		{
			for (int num = 0; num < solid.size(); num++)
			{
				...
				if (hole[num].size() > 0)
				{
					...
					CheckPoint(solid[num], pointslocation, h, Points, size);
				}
				else
				{
												...
												vector<IntPoint> &pointslocation, double size)
	{
		...
		else
		{
			...
			{
				double length = sqrt(pow((polygon[i].X - polygon[i + 1].X), 2) + pow((polygon[i].Y - polygon[i + 1].Y), 2));
				if (length > 3000)
				{
					isAddSupport = true;
					RHVector3 vector((double)(polygon[i + 1].X - polygon[i].X), (double)(polygon[i + 1].Y - polygon[i].Y), 0.0);
					int addSupportCount = (int)vector.Length() / 3000;
					vector.NormalizeSafe();
					for (int j = 1; j < addSupportCount; j++)
					{
						IntPoint spoint(polygon[i].X + 3000 * j * vector.x, polygon[i].Y + 3000 * j * vector.y);
				...
				else
				{
					...
					if (allLength > 3000)
					{
						isAddSupport = true;
						allLength = 0.0;
						IntPoint spoint((polygon[i].X + polygon[i + 1].X) / 2, (polygon[i].Y + polygon[i + 1].Y) / 2);
						supportpoints.push_back(RHVector3((double)spoint.X, (double)spoint.Y, h));
						pointslocation.push_back(spoint);
					}
				}
			}
			if (isAddSupport == false)
			{
				IntPoint spoint(0, 0);
	...
	void SupportPointsList::CheckPoint(const vector<IntPoint> polygon, const vector<IntPoint> &pointslocation, double h, vector<IntPoint> &supportPoints, double size)
	{
		for (int i = 0; i < pointslocation.size(); i++)
		{
			if (ClipperLib::PointInPolygon(pointslocation[i], polygon) != 0)
			{
				int test = 1;
	...
	void SupportPointsList::GetNeedGenerateArea(int count, /*out*/ vector<int> &controlNumber, /*out*/ vector<vector<vector<IntPoint>>> &needGenerateSupportArea)
	{
		...
		needGenerateSupportArea = ClipperGeometricOperationPolyTreeType(allUpperPolygons, noNeedSupportArea, ClipType::ctDifference, PolyFillType::pftEvenOdd, PolyFillType::pftEvenOdd);
	...
	Model: Hedgehog.stl
	Total Layers: 615
	Total FindNecessarySupportPointsNew time: 99s
	
	NOTE: This function - ClipperGeometricOperationPolyTreeType() will totally take 74s during FindNecessarySupportPointNew() process on my Asus Core i5 notebook.
	*/
	...
	int SupportPointsList::FindMeshId(IntPoint point, int level)
	{
		...
		int meshID = GetId(segment, dictionary[levelcount]);
		return meshID;
	}
	...
	// The following code is the same as SupportMeshsMark::GenMesh().
	// The GenMesh() function in SupportPointsList class should never be called because the automatically generated support points (pos, dir, radius) will be
	// delivered to SupportMeshsMark class.
	void SupportPointsList::GenMesh(shared_ptr<SupportData> &supportData)		// C#: GenMarkMesh
	{
