...
#include "RHVector3.h"
#ifdef _WINDOWS
#include "..\Helper\MyTools.h"	// for define DbgMsg().
#elif __APPLE__
#include "../Helper/MyTools.h"	// for define DbgMsg().
#endif
...
namespace XYZSupport
{
	...
	public:
		...
		// Charles: added
		Vector4		Column0() const { return Vector4(m[0], m[1], m[2], m[3]); }
		Vector4		Column1() const { return Vector4(m[4], m[5], m[6], m[7]); }
		Vector4		Column2() const { return Vector4(m[8], m[9], m[10], m[11]); }
		Vector4		Column3() const { return Vector4(m[12], m[13], m[14], m[15]); }
		Vector4		Row0() const { return Vector4(m[0], m[4], m[8], m[12]); }
		Vector4		Row1() const { return Vector4(m[1], m[5], m[9], m[13]); }
		Vector4		Row2() const { return Vector4(m[2], m[6], m[10], m[14]); }
		Vector4		Row3() const { return Vector4(m[3], m[7], m[11], m[15]); }
		...
		static Matrix4 Mult(const Matrix4 &m /*left*/, const Matrix4 &n /*right*/)
		{
		...
		//       
		static Matrix4 ToTransMatrix(const Vector3 position, const Vector3 scale, const Vector3 rotation)
		{
			Matrix4 trans;
		...
		Matrix4&	create_translation(float x, float y, float z);
		Matrix4&	create_translation(const Vector3 vector);
		Matrix4&	rotate_unit_radian(float angle, const Vector3& axis);
		Matrix4&	rotate_unit_radian(float angle, float x, float y, float z);
		//End of added

		// NOTE: the result of the C++ rotate matrix is inverse comparing C# Matrix4.CreateRotationX() function.
		...
		void ToDbgMsg(string name) const
		{
			DbgMsg("===> %s.Row0(): %s", name.c_str(), Row0().ToString().c_str());
			DbgMsg("===> %s.Row1(): %s", name.c_str(), Row1().ToString().c_str());
			DbgMsg("===> %s.Row2(): %s", name.c_str(), Row2().ToString().c_str());
			DbgMsg("===> %s.Row3(): %s", name.c_str(), Row3().ToString().c_str());
		}
