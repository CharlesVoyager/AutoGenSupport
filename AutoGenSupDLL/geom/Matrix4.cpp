...
//                         |  3  7 11 15 |
//
// Dependencies: Vector2, Vector3, Vector4
...
namespace XYZSupport
{
	...
	Matrix4& Matrix4::rotate(float angle, float x, float y, float z)
	{
		...
		/* 
		The elements of the matrix are stored as column major order.
			|  r0  r4 r8	r12 |         |  m0  m4  m8	m12 |
			|  r1  r5 r9	r13 |         |  m1  m5  m9	m13 |
			|  r2  r6 r10	r14 |         |  m2  m6 m10	m14 |
			|  r3  r7 r11	r15 |         |  m3  m7 m11	m15 |
		*/
...
/// <param name="result">The resulting Matrix4 instance.</param>
   ...
   /// <param name="result">The resulting Matrix4 instance.</param>
	Matrix4& Matrix4::create_translation(const Vector3 vector)
	{
		identity();
		m[3] = vector.x;
		m[7] = vector.y;
...
#if 0	//source: https://github.com/mono/opentk/blob/master/Source/Compatibility/Math/Matrix4.cs
...
#endif
	Matrix4& Matrix4::rotate_unit_radian(float angle, const Vector3& axis)
	{
		return rotate_unit_radian(angle, axis.x, axis.y, axis.z);
	}
	...
	/*
       ...
       |  m3  m7 m11	m15 |
	*/
	Matrix4& Matrix4::rotate_unit_radian(float angle, float x, float y, float z)	// = Matrix4.CreateFromAxisAngle
	{
		double cos = cosf(-angle);
		double sin = sinf(-angle);
		double t = 1.0f - cos;

		RHVector3 axis(x, y, z);
		axis.NormalizeSafe();

		m[0] = (float)(t * axis.x * axis.x + cos);
		m[4] = (float)(t * axis.x * axis.y - sin * axis.z);
		m[8] = (float)(t * axis.x * axis.z + sin * axis.y);
		m[12] = 0.0f;
		m[1] = (float)(t * axis.x * axis.y + sin * axis.z);
		m[5] = (float)(t * axis.y * axis.y + cos);
		m[9] = (float)(t * axis.y * axis.z - sin * axis.x);
		m[13] = 0.0f;
		m[2] = (float)(t * axis.x * axis.z - sin * axis.y);
		m[6] = (float)(t * axis.y * axis.z + sin * axis.x);
		m[10] = (float)(t * axis.z * axis.z + cos),
		m[14] = 0.0f;
		m[3] = 0;
		m[7] = 0;
		m[11] = 0;
